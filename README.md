...
122 个用例，覆盖：分块大小 / overlap / 句子边界 / 节间分隔 / 超长表格按行切与表头重复 / md 标题进正文 / 代码围栏识别、**文档解析**（docx 表格按文档顺序转 Markdown、嵌套表格拍平、单元格竖线转义、纯表格文档不丢；PDF 页级提取、layout 模式行结构还原、纯扫描件响亮失败）、混合检索 Top-K 排序与阈值过滤、BM25 + 语义两路召回与 RRF 融合（含关键词通道挽救低相似度命中）、问答 SSE 事件与消息落库、无依据处理、在线重排（重排排序 / 失败回退）、文档入库状态机（成功与失败）、文档重命名（成功 / 空名 / 不存在）、**对话记忆**（每轮事实抽取 / 超预算滚动压缩 / 追问改写驱动检索 / 记忆调用失败回退 / 关闭时零额外调用 / token 估算）、**鉴权**（图片验证码格式 / 明文不外泄 / 一次性 / 过期 / 未知 id、登录成功 / 口令错 / 未知账号 / 非 ASCII 账号不 500、token 生命周期与登出撤销、**全部业务端点的未登录 401 回归网**）、**Embedding 自适应分批**（取数请求不越服务端上限 / 跨批顺序不乱 / **收敛到真实上限而非减半近似** / 探测成本是对数级 / 第二次调用零探测开销 / 短文档一次成功 / 小文档先行不会锁死批大小 / 上限为 1 能收敛 / 无法识别的 400 原样抛出不被误重试）。测试使用独立的 `rag_kb_test` 库，Embedding / LLM / Rerank 均为 Fake 实现，**不触网、不下载模型**。
...
1. 上传文件到临时目录
2. 按类型解析为纯文本（txt/md/pdf/docx 走不同解析器；**docx 的表格会按文档顺序转成 Markdown 一并入库**；**md 的标题行本身也进正文**，否则标题不可检索）
3. 按 `chunk_size` / `overlap` 分块，在句子边界断开、保留段落语义；**节与节之间插入空行**（否则段落边界与代码围栏会被粘掉）；**表格按行切并在每块重复表头**
...
- **超长表格按行切、每块重复表头**，不切在行中间；表格块之间不重叠、也不与相邻段落合并
- **docx 表格不丢失**：按文档顺序遍历段落与表格，表格转 Markdown（保持行列对应），嵌套表格拍平进所在单元格
- **PDF 用 layout 模式提取**，同一视觉行的内容落在同一行（不再一格一行），表格行结构得以保留
- **节与节之间插入空行**：分段合并进同一 chunk 时保留原文的段落/行边界（原先直接拼接，实测仓库自己的 README.md 41 个节间边界有 38 处被粘掉，代码围栏会被粘到上一段句尾而失效）
- **md 标题进入正文**：标题行不只是元数据，也写进该节正文 —— 检索只索引 chunk 正文，标题只进 metadata 的话等于不可检索（实测 README.md：修复前 42 个节的标题在正文里出现 0 次，修复后 41 个真标题全部进入正文）
- **md 代码块按围栏识别**：围栏内的 `# 注释` 不当作标题、缩进不被压掉；围栏内的 `|…|` 行不被当成表格切分
- 解析结果为空（如纯扫描件 PDF、纯图片文档）时报错并把文档标 `failed`，**不静默入库空内容**

> **PDF 提取用 layout 模式**（按文字坐标还原版面）：同一视觉行的单元格会落在同一行、用空格分隔，表格的行结构得以保留。`plain` 模式（pypdf 默认）会按绘制顺序把同一行拆开，实测一份成绩单 PDF 出来 385 行碎片（`院(系)/部：` 被拆成 5 行），layout 模式同一文件只有 37 行。提取后会压掉冗余空格填充（16808 → 9321 字符，分块数 44 → 25）。
>
> 已知限制：**PDF 表格的列边界只是空格，不是结构** —— 列距不齐时两列会粘连（成绩单里 `22公共课/必修课` 是"总学时"与"类别"两列粘连）；且它不是 Markdown，所以**表格按行切分的逻辑对 PDF 不生效**。另外图片、图表、扫描件里的内容一律提不出来（`extract_text()` 只读文字指令）。要覆盖这些场景需引入 OCR 或版面分析，本 Demo 未做。
>
> **其余未覆盖的解析场景**（均为已知、非静默丢失）：
> - md 的**四空格缩进**代码块（非围栏写法）缩进仍会被压掉，且识别不了围栏语义 —— 可靠区分"缩进代码块"与"列表续行"需要完整的 Markdown 块级状态机，未做；
> - docx 的**页眉 / 页脚**文本不读（`iter_inner_content()` 只覆盖正文），落款、密级、版本号常放这里；
> - docx 的**自动编号**不进入段落文本（`List Number` 样式取到 `项目一` 而非 `1. 项目一`）；
> - docx 无标题元数据（段落的 Heading 样式未用于分节）。
...
- PDF 解析质量取决于扫描件还是文本型 PDF，演示素材优先选文本型；**PDF 表格能保行结构、但列只是空格，图片型表格全丢**（docx 表格则行列完整还原）
- 解析/分块只保证"内容不丢、结构可辨"，不保证还原原始版面：md 的缩进代码块、docx 的页眉页脚与自动编号都不覆盖（详见 F1.2 的已知限制）
